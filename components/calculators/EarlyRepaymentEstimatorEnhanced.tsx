...
export default function EarlyRepaymentEstimator() {
  ...
  const estimateRepaymentFee = useCallback((amount: number, yearsLeft: number): number => {
    ...
    if (yearsLeft > 10) feePercentage = 0.025      // 2.5% for long-term loans
    else if (yearsLeft > 5) feePercentage = 0.02   // 2% for medium-term
  ...
  const calculationResults = useMemo((): CalculationResults | null => {
    ...
    return {
      ...
      estimatedFee,
      breakEvenMonths,
      isWorthwhile: breakEvenMonths <= VALIDATION_RULES.BREAK_EVEN_WARNING_MONTHS && monthlySaving > 0
    }
   ...
   */
  const handleInputChange = useCallback((field: keyof LoanInputs, value: string) => {
    ...
    >
      ...
      <div className="mb-8 text-center">
        ...
        <p className="text-gray-600 text-base md:text-lg leading-relaxed max-w-3xl mx-auto">
          חשב את החיסכון הפוטנציאלי מפרעון מוקדם של המשכנתא שלך.
          החישוב מבוסס על ממוצעי בנק ישראל ומתאים למשכנתאות בישראל.
        </p>
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="text-center p-4 bg-white rounded-lg shadow-sm">
              <div className="text-3xl md:text-4xl font-bold text-blue-600 mb-2">
                {calculationResults.breakEvenMonths === Infinity ? '∞' : calculationResults.breakEvenMonths}
              </div>
              <div className="text-sm text-gray-600 font-medium">
                נקודת איזון (חודשים)
              </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <h4 className="font-semibold mb-2">
                  {calculationResults.isWorthwhile ? 'המלצה: שווה לשקול פרעון מוקדם' : 'המלצה: כדאי לבחון חלופות נוספות'}
                </h4>
          ...
          <div className="mt-6 text-xs text-gray-500 border-t pt-4">
            <p className="leading-relaxed">
              * החישוב הוא הערכה בלבד ואינו מהווה ייעוץ פיננסי. עמלות הפרעון המוקדם משתנות בין הבנקים השונים.
              מומלץ לקבל הצעות מחיר מדויקות מהבנק שלכם ולהתייעץ עם יועץ משכנתאות מוסמך.
            </p>
          </div>
        </div>
      )}
