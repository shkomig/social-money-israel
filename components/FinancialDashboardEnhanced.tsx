...
function calculateTotalBenefit(
  ...
  try {
    const selectedResults =
      selectedIds.length > 0 ? results.filter((r) => selectedIds.includes(r.id)) : results
...
function useNotificationManager() {
  ...
  const scheduleReminder = useCallback(
    (deadline: DeadlineItem, daysBeforeWarning: number = 7) => {
      const deadlineDate = new Date(deadline.date)
      const reminderDate = new Date(
        deadlineDate.getTime() - daysBeforeWarning * 24 * 60 * 60 * 1000,
      )
      const now = new Date()
...
export default function FinancialDashboard({
  ...
  const { permission, settings, setSettings, requestPermission, scheduleReminder } =
    ...
    >
      ...
      <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-6 md:p-8">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
          <div>
            <h1 id="dashboard-title" className="text-2xl md:text-3xl font-bold text-gray-900 mb-2">
              הלוח הפיננסי שלי
            </h1>
          ...
          <div className="bg-gradient-to-br from-green-50 to-blue-50 rounded-xl border border-green-200 p-6 md:p-8">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              <div>
                ...
                <p className="text-sm text-gray-600">לשנה (כולל חיסכונים ממחשבונים)</p>
              </div>
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-2 text-sm text-gray-600">
                          ...
                          {Array.isArray(result.requirements) && (
                            <span className="text-xs">{result.requirements.length} תנאים</span>
                          )}
                        </div>
                      </div>
                    ...
                    <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                      <div className="flex-1">
                        ...
                        <div className="flex flex-wrap items-center gap-3 mb-3 text-sm">
                          <time dateTime={deadline.date} className="font-medium text-gray-900">
      ...
      {activeView === 'settings' && (
        ...
        >
          <div className="bg-white rounded-xl border border-gray-200 p-6 md:p-8">
            <h2 className="text-xl md:text-2xl font-semibold text-gray-900 mb-6">הגדרות התראות</h2>
            ...
            <div className="space-y-6">
                ...
                {[
                  {
                    ...
                    description: 'קבל התראה לפני תאריכי יעד חשובים',
                  },
                  {
                    key: 'benefitUpdates',
                    label: 'עדכוני הטבות',
                    description: 'התראות על הטבות חדשות או שינויים בקיימות',
                  },
                  {
                    key: 'calculatorAlerts',
                    label: 'התראות מחשבונים',
                    description: 'התראות על הזדמנויות חיסכון מהמחשבונים',
                  },
                ].map((setting) => (
                  <label
                    key={setting.key}
                  ...
                  >
                    ...
                    <div>
                      ...
                      <div id={`${setting.key}-description`} className="text-sm text-gray-600">
                        {setting.description}
                      </div>
                    </div>
                  </label>
              ...
              {permission !== 'granted' && (
                ...
                >
                  ...
                  {permission === 'denied' ? 'אפשר התראות בהגדרות הדפדפן' : 'אפשר התראות'}
                </button>
              )}
            </div>
          </div>
